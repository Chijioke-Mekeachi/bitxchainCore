...
export default function Wallet() {
  ...
  useEffect(() => {
    const resolve = async () => {
      ...
      setWithdrawBankUsername(name);
    };

    resolve();
  }, [withdrawAccountNumber, withdrawBankName, banks]);
  const resolveAccountName = async (accountNumber, bankNameOrCode, banksList) => {
    const selectedBank = banksList.find(
      (bank) =>
        bank.name.toLowerCase() === bankNameOrCode.toLowerCase() ||
        bank.code === bankNameOrCode
    );

    if (selectedBank && accountNumber.length === 10) {
      try {
        const response = await fetch(
          `https://bitapi-0m8c.onrender.com/api/resolve_account?account_number=${accountNumber}&bank_code=${selectedBank.code}`
        );
  ...
  useEffect(() => {
    fetch("https://bitapi-0m8c.onrender.com/api/get_banks_code")
      .then((res) => res.json())
      .then(setBanks)
      .catch(console.error);
  }, []);
          ...
          <div className="popup-overlay" onClick={() => setShowWithdraw(false)}>
            <div className="popup" onClick={(e) => e.stopPropagation()}>
              ...
              >
                ...
                {banks.map((bank, idx) => (
                  <option key={`${bank.code}-${idx}`} value={bank.name}>
                    {bank.name}
                  </option>
                ))}
              </select>
